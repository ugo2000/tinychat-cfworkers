...
export class ChatRoom {
  ...
  async handleChatMessage(senderWs, username, text) {
    ...
    if (quota === 0) {
      try { await senderWs.send(JSON.stringify({ type: 'system', code: 'QUOTA_EXHAUSTED', text: '? })); } catch(e) {}
      return;
    }
    ...
    if (quota === 0) {
      for (const ws of this.state.getWebSockets()) {
        ...
        if (a && a.username === from) {
          try { await ws.send(JSON.stringify({ type: 'system', code: 'QUOTA_EXHAUSTED', text: '? })); } catch(e) {}
        }
      }
      return;
    }
  ...
  async handleBuy(request) {
    ...
    let label = '', price = 0;
    if (pkg === 'sub') { label = '?; price = 29.9; }
    else if (pkg === 'sub_year') { label = '?; price = YEAR_PRICE; }
    ...
    return json({ ok: true, quota: -1, mock: true, pkg, label, price, note: '? });
  }
  ...
  // ?pwd ?Worker ?  async handlePayQr(request) {
