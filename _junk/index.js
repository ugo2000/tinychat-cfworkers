...
// 管理后台密码?Cloudflare secret ADMIN_PASSWORD 提供（wrangler secret put ADMIN_PASSWORD?// 改密码免改代码：执行 `wrangler secret put ADMIN_PASSWORD` 后重新部署即生效
// ===================== Worker 入口 =====================
export default {
  async fetch(request, env) {
    ...
    // 个人收款码配置：转发?DO（DO 才能?storage 里存的收款码?    if (path === '/api/pay-config') {
      const stub = env.CHAT.idFromName('global12');
      return env.CHAT.get(stub).fetch(request);
    }

    // 管理员上?查询收款码图片（pwd 鉴权?    if (path === '/api/pay-qr') {
...
export class ChatRoom {
  ...
  async handleChatMessage(senderWs, username, text) {
    ...
    if (quota === 0) {
      try { await senderWs.send(JSON.stringify({ type: 'system', code: 'QUOTA_EXHAUSTED', text: '免费额度已用完，请订阅或一次性购买后继续发送? })); } catch(e) {}
      return;
    }
  ...
  async handlePrivateMessage(from, to, text) {
    ...
    if (quota === 0) {
      for (const ws of this.state.getWebSockets()) {
        ...
        if (a && a.username === from) {
          try { await ws.send(JSON.stringify({ type: 'system', code: 'QUOTA_EXHAUSTED', text: '免费额度已用完，请订阅或一次性购买后继续发送? })); } catch(e) {}
        }
      }
      return;
    }
    ...
    // 私聊只发给接收方(to)，发件人(from)靠前端乐观更新显示，避免重复
  ...
  async handleBuy(request) {
    ...
    // 所有套餐均为无限条；一次性购买价?= 年度价格 + 200
    const YEAR_PRICE = 299;
    let label = '', price = 0;
    if (pkg === 'sub') { label = '月度订阅（无限条?; price = 29.9; }
    else if (pkg === 'sub_year') { label = '年度订阅（无限条?; price = YEAR_PRICE; }
    else { label = '一次性购买（无限条）'; price = YEAR_PRICE + 200; }

    // 真实微信支付已配置：创建 Native 支付订单并返回二维码
    if (wxConfigured(this.env)) {
      try {
        ...
        return json({ ok: false, error: '微信下单失败? + e.message }, 500);
      }
    }

    // 个人收款码模式（无需商户号）：返回收款二维码，用户扫码后手动确认
    const payWechat = this.env.PAY_WECHAT_QR || '';
    const payAlipay = this.env.PAY_ALIPAY_QR || '';
    // 优先 env secret；否则读管理员上传到 storage 的二维码
    ...
    // 未配置任何真实支付：回退模拟
    await this.setQuota(payload.username, -1); // -1 = 无限?    await this.pushQuota(payload.username);
    return json({ ok: true, quota: -1, mock: true, pkg, label, price, note: '模拟支付成功（真实支付未接入? });
  }

  // 个人收款码：用户确认已付??记录待审?  async handlePayConfirm(request) {
    const body = await request.json().catch(() => ({}));
    const payload = await verifyToken(body.token);
    if (!payload) return json({ ok: false, error: 'Unauthorized' }, 401);
    const pkg = body.pkg || 'once';
    const pending = await this.state.storage.get('payPending') || [];
    // 同用户同套餐重复提交则更?timestamp
  ...
  // 用户查询自己的待审核状?  async handlePayPending(request) {
    const body = await request.json().catch(() => ({}));
    const payload = await verifyToken(body.token);
    if (!payload) return json({ ok: false, error: 'Unauthorized' }, 401);
    const pending = await this.state.storage.get('payPending') || [];
  ...
  // 管理员上?查询收款码（pwd 鉴权?Worker 层已完成?  async handlePayQr(request) {
  ...
  async handleWxMark(url) {
    ...
    orders[outTradeNo] = o;
    await this.state.storage.put('wxOrders', orders);
    await this.setQuota(o.username, -1); // -1 = 无限条（含订?一次性）
    await this.pushQuota(o.username);
    return json({ ok: true });
  }
}

// 微信支付回调（Worker 层处理，校验+解密后写?DO 订单状态）
async function handleWechatNotify(request, env) {
  ...
  try {
    ...
    if (plain.trade_state === 'SUCCESS') {
      ...
      if (j.ok) {
        return new Response(JSON.stringify({ code: 'SUCCESS', message: '成功' }), { headers: { 'Content-Type': 'application/json' } });
      }
    }
  } catch (e) { return new Response('decrypt failed', { status: 500 }); }
  return new Response(JSON.stringify({ code: 'SUCCESS', message: '已接? }), { headers: { 'Content-Type': 'application/json' } });
}
